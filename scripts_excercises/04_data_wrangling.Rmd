...
mean_examplevector <- mean(example_vector)
round(mean_examplevector, 2)
```
...
```{r}
#shortcut for Pipe Operator %>%  = Alt + .
```
...
```
There are arguments in the brackets, they are just invisible - its the variable name at the beginning. You can use a dot as a place holder.
Normally, everything that happens gets its own line so it is more visible what's happening.
...
# alternativ nerd_red %>% 
#  filter(gender == 3, !married == 1 )
# alles außer never been marriedM man kann auch >=2 nehmen 
...
```{r}
nerd_red %>% 
  group_by(gender, nerdy) %>% 
  count()

#count(name = "number of people") benennt dann Variable in Tabelle anders
# mit nerd_table <- kann man tabelle quasi speichern, mit nerd_table dann wieder abrufen
```
...
```
Pull extracts as a vector and select extracts as a dataframe/table.
...
nerd_red <- 
  nerd_red %>% 
  mutate(age_character = as.numeric(age_character), .before = gender)
...
nerd_red %>% 
  mutate(sum_Q1_Q2 = sum(Q1,Q2), .after = Q2)
